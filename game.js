var Message = new Class({
    ...
    _addEvents: function() {
        ...
        .bind(this));
        this.container.getElements(".action-facebook").addEvent("click", function(a) {
...
var GameInterface = new Class({
    ...
    startGame: function(b) {
        ...
        a.addEvent("action", function(c, e) {
            ...
            var f = function(i, h) {
                ...
                .bind(this), function() {
                    this.startLock = false;
                    c.getElement(".error").set("html", "We couldn't find any roads around that location for creeps to move along.  Please try somewhere else.");
            ...
            this.g.geocode(d, f, function() {
                this.startLock = false;
                c.getElement(".error").set("html", "We couldn't find that location.  Please try to be more specific, or search for somewhere else.");
                c.getElement(".error").show()
            }
            .bind(this))
        }
    ...
    _initPause: function() {
        this.container.getElement(".controls .pause").addEvent("click", function(a) {
    ...
    _initFastForward: function() {
        this.container.getElement(".controls .fast-forward").addEvent("click", function(a) {
    ...
    _initNextRound: function() {
        ...
        this.container.getElement(".controls .next-round .button").addEvent("click", function() {
    ...
    nextRound: function() {
        this.container.getElement(".controls .next-round").setStyle("visibility", "visible").set("slide", {
            duration: "long",
            ...
            var g = function(l) {
                ...
                i = {
                    ...
                    mousemove: function(r) {
                        ...
                        var u = a.getProjection().fromContainerPixelToLatLng(new google.maps.Point(t - j.x + ((controlSize.x / 2) - iconX),s - j.y + (controlSize.y - iconY)));
    ...
    showTowerInfoWin: function(b) {
        ...
        a.getElements(".upgrade").each(function(c, e) {
            ...
            b.upgrades[e].each(function(h) {
                ...
                var i = this.container.getElement(".hidden-elements .control-tower-infowin-upgrade .content").clone();
    ...
    updateTowerInfoWin: function() {
        this.container.getElements(".tower-infowin-container .upgrade .button").each(function(a) {
    ...
    win: function() {
        ...
        .bind(this));
        a.container.getElement(".action-twitter").set("data-text", "I made it to round 50 on #MapsTowerDefence!").addClass("twitter-share-button");
    ...
    lose: function() {
        ...
        .bind(this));
        a.container.getElement(".action-twitter").set("data-text", "I made it to round " + this.g.moneyRounds + " on #MapsTowerDefence!").addClass("twitter-share-button");
        twitter(document, "script", "twitter-wjs-" + new Date());
        a.addEvent("social", function(b) {
            switch (b) {
            case "facebook":
    ...
    _fullscreenPopup: function(b, c, a) {
        return new Message("fullscreen",b,this.container.getElement(".hidden-elements").getChildren("." + c)[0].clone(),a)
    },
    initMiscButtons: function(a) {
        ...
        a.getElements(".button.chat").addEvent("click", function(b) {
            var c = window.open("http://webchat.quakenet.org/?channels=MapsTD&uio=d4", "MapsTD Chat", "width=647,height=400");
            if (window.focus) {
                c.focus()
            }
        }
...
var RouteBuilder = new Class({
    ...
    generate: function(b, a) {
        ...
        this.tmpRoutes = {};
        new Route(this.g,this.start,new google.maps.LatLng(this.start.lat() + 0.02,this.start.lng()),function(c) {
            this._add(c, "north")
        }
        .bind(this),function() {
            this._fail()
        ...
        .bind(this));
        new Route(this.g,this.start,new google.maps.LatLng(this.start.lat(),this.start.lng() + 0.04),function(c) {
            this._add(c, "east")
        }
        .bind(this),function() {
            this._fail()
    ...
    _addedRoute: function() {
        ...
        for (var a = 1; a < this.routes.length; a++) {
            this.routes[a].addMessage = ["top", "success", "Let's make this a bit harder.  <b>New route added to the " + this.routes[a].compass + ".</b>", {
                autoHide: 15000
            }]
        }
        var a = 0;
        while (this.routes.length < 4) {
            ...
            b.addMessage = ["top", "error", "We couldn't find another route to add to the map, so we've made the " + b.compass + " path harder.  Have fun :)", {
                autoHide: 10000
            }];
            this.routes.push(b)
        }
...
var Route = new Class({
    ...
    enable: function() {
        ...
        this.marker = new google.maps.Marker({
            ...
            map: this.g.map,
            icon: new google.maps.MarkerImage(this.g.search.image.routeend,new google.maps.Size(32,32),new google.maps.Point(0,32),new google.maps.Point(16,32))
        });
        return this
    },
    remove: function() {
...
var Creep = new Class({
    ...
    show: function() {
        this.marker = new google.maps.Marker({
            ...
            icon: new google.maps.MarkerImage(this.g.search.image.creeps,new google.maps.Size(32,32),new google.maps.Point(0,(this.markerOffset + 12) * 32),new google.maps.Point(16,32)),
            map: this.g.map,
            shadow: new google.maps.MarkerImage("images/creep-health.png",new google.maps.Size(16,8),new google.maps.Point(0,0),new google.maps.Point(8,32))
        });
        return this
    },
    move: function() {
...
var Tower = new Class({
    ...
    initialize: function(a, b) {
        this.g = a;
        this.marker = new google.maps.Marker({
            icon: new google.maps.MarkerImage(this.g.search.image.towers,new google.maps.Size(32,32),new google.maps.Point(0,(this.markerOffset + 2) * 32),new google.maps.Point(16,32)),
            position: b,
            visible: false,
            map: this.g.map
        });
    ...
    calculateRouteOverlap: function() {
        this.g.routes.each(function(b, a) {
            ...
            b.path.each(function(c) {
                if (google.maps.geometry.spherical.computeDistanceBetween(c.latLng, this.marker.getPosition()) < this.attackRadius) {
    ...
    checkDroppable: function() {
        ...
        if (a) {
            this.g.towers.each(function(b) {
                ...
                if (google.maps.geometry.spherical.computeDistanceBetween(b.marker.getPosition(), this.marker.getPosition()) < this.size + b.size) {
                    a = false
                }
            }
            .bind(this))
        ...
        if (a) {
            this.g.foundRoutes.routes.each(function(b) {
                b.path.each(function(c) {
                    if (google.maps.geometry.spherical.computeDistanceBetween(c.latLng, this.marker.getPosition()) < this.size) {
                        a = false
                    }
                }
                .bind(this))
...
var Rounds = new Class({
    ...
    get: function(roundNum) {
        ...
        return new GameRound(function() {
            ...
            for (var creepType = 1; creepType <= this.g.rounds.creepTypes.length; creepType++) {
                ...
                var creeps = Math.max(0, Math.round(Math.sin((Math.PI * (roundNum + 5 - (this.g.rounds.roundNumModifier * 2)) / 40) - (Math.PI * (creepType - 1) / 5)) * 30 / this.g.rounds.roundNumModifier), 0);
                for (var routeIndex = 0; routeIndex < this.g.routes.length; routeIndex++) {
                    for (var i = 0; i < creeps; i++) {
                        this.addCreep(this.g.routes[routeIndex], this.g.rounds.creepTypes[creepType - 1], 500 * ++timerCount[routeIndex])
                    }
                }
            }
        }
    ...
    _calc: function(b, a) {
        return Math.round((Math.sin(((b - a) % 10) * 6 / a) + 1) * (b / Math.pow((a + 1), 1.7)) + (1.3 ^ (b / (13 - (a % 10))) / 10)) + "@2"
    },
